...
from tensorflow.keras.layers import Conv3D, MaxPool3D, ZeroPadding3D, AveragePooling3D, GlobalAveragePooling3D
from tensorflow.keras.layers import Dense, Dropout, Flatten, Input, Activation, BatchNormalization, Reshape, Lambda
...
# PATHS: to pretrained models with top (classification layer)
WEIGHTS_PATH = {
...
# PATHS: to pretrained models with no top (no classification layer)
WEIGHTS_PATH_NO_TOP = {
...
def _obtain_input_shape(input_shape, default_frame_size, min_frame_size, default_num_frames, min_num_frames,
                        data_format, require_flatten, weights=None):
    """Internal utility to compute/validate the model's input shape.
    (Adapted from `keras/applications/imagenet_utils.py`)
    # Arguments
        input_shape: either None (will return the default network input shape), or a user-provided shape to be validated.
        ...
        require_flatten: whether the model is expected to be linked to a classifier via a Flatten layer.
...
def conv3d_bn(x, filters, num_frames, num_row, num_col, padding='same', strides=(1, 1, 1), use_bias = False, use_activation_fn = True,
              use_bn = True, name=None):
...
def Inception_Inflated3d(include_top=True,
    ...
    # Arguments
        include_top: whether to include the the classification layer at the top of the network.
        weights: one of `None` (random initialization)
        ...
        dropout_prob: optional, dropout probability applied in dropout layer after global average pooling layer. 
            0.0 means no dropout is applied, 1.0 means dropout is applied to all features.
            Note: Since Dropout is applied just before the classification layer, it is only useful when `include_top` is set to True.
    ...
    x = MaxPool3D((1, 3, 3), strides=(1, 2, 2), padding='same', name='MaxPool2d_2a_3x3')(x)
    x = conv3d_bn(x, 64, 1, 1, 1, strides=(1, 1, 1), padding='same', name='Conv3d_2b_1x1')
    x = conv3d_bn(x, 192, 3, 3, 3, strides=(1, 1, 1), padding='same', name='Conv3d_2c_3x3')

    # Downsampling (spatial only)
    x = MaxPool3D((1, 3, 3), strides=(1, 2, 2), padding='same', name='MaxPool2d_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_3b_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_3c_3a_3x3')(x)
    branch_3 = conv3d_bn(branch_3, 64, 1, 1, 1, padding='same', name='Conv3d_3c_3b_1x1')

    x = layers.concatenate([branch_0, branch_1, branch_2, branch_3], axis=channel_axis, name='Mixed_3c')

    # Downsampling (spatial and temporal)
    x = MaxPool3D((3, 3, 3), strides=(2, 2, 2), padding='same', name='MaxPool2d_4a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4b_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4c_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4d_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4e_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4f_3a_3x3')(x)
    branch_3 = conv3d_bn(branch_3, 128, 1, 1, 1, padding='same', name='Conv3d_4f_3b_1x1')

    x = layers.concatenate([branch_0, branch_1, branch_2, branch_3], axis=channel_axis, name='Mixed_4f')

    # Downsampling (spatial and temporal)
    x = MaxPool3D((2, 2, 2), strides=(2, 2, 2), padding='same', name='MaxPool2d_5a_2x2')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_5b_3a_3x3')(x)
    ...
    branch_3 = MaxPool3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_5c_3a_3x3')(x)
...
def i3d_feature_extractor():
    model = Inception_Inflated3d(include_top=False,
                weights='rgb_kinetics_only', input_tensor=None,
                input_shape=(16,112,112,3), dropout_prob=0.0,
                endpoint_logit=True, classes=400)
